...
// the search LEFT JOINs to a query. Tables are joined for search even when
// the search string is empty so the column references in WHERE remain
// resolvable; GORM elides the join from the executed plan when no
// reference is present. The join cardinality is 1:1 on FK columns, so
...
func applyMCPSessionFilters(query *gorm.DB, params MCPSessionsFilterParams, t mcpSessionFilterTable) *gorm.DB {
	...
	if params.Search != "" {
		...
		query = query.
			...
			Where(
				"LOWER(config_mcp_clients.name) LIKE ? OR LOWER(config_mcp_clients.client_id) LIKE ? OR LOWER("+t.table+".user_id) LIKE ? OR LOWER("+t.table+".session_id) LIKE ? OR LOWER(governance_virtual_keys.id) LIKE ? OR LOWER(governance_virtual_keys.name) LIKE ?",
