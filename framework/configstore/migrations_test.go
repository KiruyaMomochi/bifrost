...
// TestMigrationAddVirtualKeyExpiresAtColumn verifies that the expires_at
// migration adds a nullable column, leaves existing rows with NULL, and is
// idempotent.
//
// This test simulates a pre-migration database by creating governance_virtual_keys
// without the expires_at column via a raw CREATE TABLE statement, mirroring the
// state an existing deployment would be in before the migration runs.
func TestMigrationAddVirtualKeyExpiresAtColumn(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{
		Logger: logger.Default.LogMode(logger.Silent),
	})
	require.NoError(t, err)
	...
	// Create the migrations tracking table that the migrator library expects.
	require.NoError(t, db.Exec(`CREATE TABLE IF NOT EXISTS migrations (id VARCHAR(255) PRIMARY KEY)`).Error)

	// Create governance_virtual_keys WITHOUT expires_at to simulate pre-migration state.
	...
	// Insert a row so we can verify NULL backfill.
	now := time.Now().UTC().Format("2006-01-02 15:04:05")
	require.NoError(t, db.Exec(
		`INSERT INTO governance_virtual_keys (id, name, value, is_active, created_at, updated_at, encryption_status, value_hash)
		 VALUES (?, ?, ?, 1, ?, ?, 'plain_text', ?)`,
		"vk-expiry-test", "expiry-test", "sk-bf-expiry", now, now, "abc123",
	).Error, "insert pre-migration row")

	// Column should not exist yet.
	assert.False(t, db.Migrator().HasColumn(&tables.TableVirtualKey{}, "expires_at"),
		"expires_at should not exist before migration")

	// Run the migration.
	require.NoError(t, migrationAddVirtualKeyExpiresAtColumn(ctx, db))

	// Column must exist now.
	assert.True(t, db.Migrator().HasColumn(&tables.TableVirtualKey{}, "expires_at"),
		"expires_at should exist after migration")

	// No index should have been created (enforcement is purely in-memory).
	assert.False(t, db.Migrator().HasIndex(&tables.TableVirtualKey{}, "idx_expires_at"),
		"no index should be created on expires_at")

	// Existing row should have NULL expires_at.
	var expiresAt *string
	require.NoError(t, db.Table("governance_virtual_keys").
		Select("expires_at").
		Where("id = ?", "vk-expiry-test").
		Scan(&expiresAt).Error)
	assert.Nil(t, expiresAt, "existing rows should have NULL expires_at after migration")
