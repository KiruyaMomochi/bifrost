...
// Empty values are dropped so RTK Query's cache key doesn't fragment on
// "" vs unset.
function buildMCPSessionsListParams(params?: MCPSessionsQueryParams) {
	if (!params) return {};
	const out: Record<string, string | number> = {};
	if (params.q) out.q = params.q;
	if (params.kind?.length) out.kind = params.kind.join(",");
	if (params.status?.length) out.status = params.status.join(",");
	if (params.auth_mode?.length) out.auth_mode = params.auth_mode.join(",");
	if (params.mcp_client_id?.length) out.mcp_client_id = params.mcp_client_id.join(",");
