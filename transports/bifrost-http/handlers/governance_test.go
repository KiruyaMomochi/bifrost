...
// TestCreateVirtualKey_ExpiresAtValidation covers expires_at rejection on the
// create path. Only the 400 error cases are tested here; the success path
// requires a full config store and is covered by integration tests.
func TestCreateVirtualKey_ExpiresAtValidation(t *testing.T) {
	SetLogger(&mockLogger{})
	h := &GovernanceHandler{}
	...
	tests := []struct {
		...
		{
			name:     "past expires_at is rejected",
			body:     `{"name":"test","expires_at":"` + past.Format(time.RFC3339) + `"}`,
			wantBody: "expires_at must be a future timestamp",
		},
		{
			name:     "current time expires_at is rejected",
			body:     `{"name":"test","expires_at":"` + now.Format(time.RFC3339) + `"}`,
			wantBody: "expires_at must be a future timestamp",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(string(ctx.Response.Body()), tt.wantBody) {
				t.Fatalf("body %q does not contain %q", ctx.Response.Body(), tt.wantBody)
			}
		})
	}
}

// TestUpdateVirtualKey_ExpiresAtValidation covers expires_at rejection on the
// update path: mutual exclusion with clear_expires_at, and past timestamps.
// Only the 400 error cases are tested here.
func TestUpdateVirtualKey_ExpiresAtValidation(t *testing.T) {
	SetLogger(&mockLogger{})
	h := &GovernanceHandler{}
	...
	tests := []struct {
		...
		{
			name:     "both expires_at and clear_expires_at is rejected",
			body:     `{"expires_at":"` + future.Format(time.RFC3339) + `","clear_expires_at":true}`,
			wantBody: "cannot set both expires_at and clear_expires_at",
		},
		{
			name:     "past expires_at is rejected",
			body:     `{"expires_at":"` + past.Format(time.RFC3339) + `"}`,
			wantBody: "expires_at must be a future timestamp",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctx := &fasthttp.RequestCtx{}
			ctx.SetUserValue("vk_id", "vk-test-id")
			ctx.Request.SetBody([]byte(tt.body))
